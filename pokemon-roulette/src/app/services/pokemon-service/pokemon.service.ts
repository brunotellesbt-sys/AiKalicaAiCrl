...
import {catchError, map, Observable, retry, throwError, of} from 'rxjs';
...
export class PokemonService {
  ...
  /**
   * Fetches sprites by PokeAPI pokemon name (e.g. "groudon-primal").
   * Useful for special forms when we don't have a stable numeric id locally.
   */
  getPokemonSpritesByName(pokemonName: string): Observable<{ sprite: { front_default: string; front_shiny: string } }> {
    const name = (pokemonName ?? '').toString().trim().toLowerCase();
    if (!name) {
      return throwError(() => new Error('Invalid Pokémon name'));
    }
    ...
    return this.http.get<any>(url).pipe(
      ...
      map((response) => {
        ...
        return { sprite: { front_default, front_shiny } };
      }),
      catchError((error) => {
   ...
   * @param pokemonId The ID of the Pokémon.
   * @returns An Observable of type names (lowercase), e.g. ["fire","flying"].
   */
  getPokemonTypes(pokemonId: number): Observable<string[]> {
    ...
    return this.http.get<any>(url).pipe(
      ...
      retry(1),
      catchError((err) => {
        console.error('getPokemonTypes failed', pokemonId, err);
        return of([] as string[]);
      })
    );
  }
